...
v0.2, 2026-08-23: Cross-platform cache locations
:description: Wide-format thumbnail management specification
...
of this document's conception (end of year 2021) show previews in a square
grid.  This wastes a lot of display real estate with padding, and looks
subjectively awkward.  The Web at large has long been moving away from this
...
Storage
-------
The user-specific base directory for thumbnails is:

 - on macOS, _~/Library/Caches/thumbnails_,
 - on Windows, _%LOCALAPPDATA%/cache/thumbnails_,
 - other systems follow the XDG Base Directory Specification:
   _$XDG_CACHE_HOME/thumbnails_, defaulting to _~/.cache/thumbnails_.

The list of subdirectories is similar to the original specification,
but a `wide-` prefix is added, turning `large` into `wide-large`, and so on.
The `fail` directory does not constitute an exception to this rule,
and is also duplicated, if necessary.
...
kept, but a factor of 2 is applied to the _width_:
 ...
 - _wide-normal_ contains previews proportionally scaled down to 256x128 pixels,
 - _wide-large_ contains 512x256 pixel previews,
 - _wide-x-large_ contains 1024x512 pixel previews,
 - _wide-xx-large_ contains 2048x1024 pixel previews.
...
names from the MD5 hash of input URIs, as in the original standard, because
...
If converting, you should pick the color space in such a way as to not crop
the original gamut unnecessarily.

Interactions
------------
Programs may fall back to picking up and rescaling a square-sized thumbnail if
they fail to find a wide one, preferably one size above what they are looking
...
Change history
--------------
v0.2, 2026-08-23, Přemysl Eric Janouch::
	Define special cache roots for macOS and Windows.
	Suggest wider color gamuts.
