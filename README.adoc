...
Building
--------
Debian 13 Trixie instructions, which you may derive as appropriate:

```sh
# Required dependencies
apt install build-essential cmake qt6-base-dev glslang-tools librsvg2-bin libwebp-dev libjpeg-dev libcolord-dev shared-mime-info
# Optional dependencies
apt install librsvg2-dev libxcursor-dev libheif-dev libtiff-dev libraw-dev libgdk-pixbuf-2.0-dev qt6-wayland-dev wayland-protocols
...
tar xJf $archive
cargo cinstall --release --prefix $PWD/resvg-prefix --manifest-path resvg-$ver/crates/c-api/Cargo.toml

# The application will run much faster if you allow bundling Little CMS
cmake -S . -B build -DCMAKE_BUILD_TYPE=Release -DDAWN_BUNDLED_LCMS2=ON -DCMAKE_PREFIX_PATH=$PWD/resvg-prefix
cmake --build build
```
