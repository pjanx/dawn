...
struct Viewer : Widget {
	...
	const Image *info_text_src_ = nullptr;
	QString scale_text_;
	std::string message_;
	...
	void set_screen_profile(std::shared_ptr<Cmm> cmm,
		std::shared_ptr<Profile> profile, bool fallback = true);
