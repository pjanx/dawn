//
// kit-commander.cpp: orthodox file manager subapplication
//
...
unique_ptr<Page>
make_commander_page(Kit &kit, const HostActions &host, Commander **out)
{
	auto content = make_unique<Commander>();
	if (out)
		*out = content.get();

	PageSetup setup;
	setup.mode = Mode::Commander;
